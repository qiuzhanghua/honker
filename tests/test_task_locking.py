"""Tests for db.lock() / honker.LockHeld.
...
def test_lock_acquire_and_release(db_path):
    ...
    with db.lock("backup", ttl=60) as lk:
        assert lk.acquired
        rows = db.query("SELECT name, owner FROM _honker_locks")
        assert len(rows) == 1
        assert rows[0]["name"] == "backup"

    # Released on exit.
    rows = db.query("SELECT COUNT(*) AS c FROM _honker_locks")
    assert rows[0]["c"] == 0
...
def test_lock_expired_is_reclaimable(db_path):
    ...
    with db.lock("stale", ttl=60) as lk:
        assert lk.acquired
        rows = db.query("SELECT owner FROM _honker_locks WHERE name='stale'")
