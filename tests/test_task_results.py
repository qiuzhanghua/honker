...
def test_enqueue_in_tx_returns_id(db_path):
    ...
    # Row actually landed.
    rows = db.query(
        "SELECT id FROM _honker_live WHERE queue='tx-id'"
    )
...
def test_expired_result_reads_as_missing(db_path):
    ...
    rows = db.query("SELECT COUNT(*) AS c FROM _honker_results")
    assert rows[0]["c"] == 1
...
def test_sweep_results_only_expired(db_path):
    ...
    assert q.sweep_results() == 1
    rows = db.query(
        "SELECT job_id FROM _honker_results ORDER BY job_id"
    )
...
async def test_run_task_save_result_disabled_by_default(db_path):
    ...
    jobs = q.claim_batch("w1", 1)
    await honker._worker.run_task(jobs[0], handler)
...
async def test_run_task_failed_handler_does_not_save_result(db_path):
    ...
    is taken and nothing lands in _honker_results. Callers treat
