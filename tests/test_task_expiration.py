...
def test_no_expires_means_never_expires(db_path):
    ...
    rows = db.query(
        "SELECT expires_at FROM _honker_live WHERE queue='exp-none'"
    )
...
def test_sweep_expired_moves_to_dead(db_path):
    ...
    live = db.query(
        "SELECT COUNT(*) AS c FROM _honker_live WHERE queue='exp-sweep'"
    )[0]["c"]
    assert live == 2  # {i:3} and {i:4} remain

    dead = db.query(
        "SELECT COUNT(*) AS c, last_error "
        "FROM _honker_dead WHERE queue='exp-sweep' GROUP BY last_error"
    )
    assert dead[0]["c"] == 2
...
def test_enqueue_with_tx_sets_expires(db_path):
    ...
    rows = db.query("SELECT expires_at FROM _honker_live WHERE queue='exp-tx'")
